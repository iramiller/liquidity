...
func NewConfig(dbm *dbm.MemDB) network.Config {
	encCfg := liquidityparams.MakeTestEncodingConfig()

	cfg := network.DefaultConfig()
	cfg.AppConstructor = NewAppConstructor(encCfg, dbm)                           // the ABCI application constructor
	cfg.GenesisState = liquidityapp.ModuleBasics.DefaultGenesis(encCfg.Marshaler) // liquidity genesis state to provide
	return cfg
}

// NewAppConstructor returns a new network AppConstructor.
func NewAppConstructor(encodingCfg liquidityparams.EncodingConfig, dbm *dbm.MemDB) network.AppConstructor {
	return func(val network.Validator) servertypes.Application {
		return liquidityapp.NewLiquidityApp(
			val.Ctx.Logger, dbm, nil, true, make(map[int64]bool), val.Ctx.Config.RootDir, 0,
			encodingCfg,
